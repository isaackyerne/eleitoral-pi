...
import { ATRIBUICAO_CARTO, CENTRO_BA, urlTilesCarto, ZOOM_BA } from '../viz/mapabase'

/**
 * Mapa da Bahia sobre tiles reais (CARTO Positron, atribuição OpenStreetMap).
 *
...
export function Mapa({
  ...
  useEffect(() => {
    fetch(`${import.meta.env.BASE_URL}dados/municipios_ba.geojson`)
      .then((r) => r.json())
      .then(setGeo)
      .catch(() => setGeo(null))
  }, [])
      ...
      <div className="relative grid gap-4 p-4 lg:grid-cols-[minmax(0,1fr)_200px]">
        <div className="relative overflow-hidden rounded-lg" style={{ height: ALTURA }}>
          {!geo || carregando ? (
            ...
            <MapContainer
              center={CENTRO_BA} zoom={ZOOM_BA} minZoom={5} maxZoom={16}
              style={{ height: '100%', width: '100%', background: t.plano }}
              aria-label="Mapa da Bahia por município"
            >
